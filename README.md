...
## Installing BepInEx
[BepInEx] is a modding framework that allows mods to be created for Unity games. Among Us uses Unity as their game engine, so Hydra uses the BepInEx modding framework in order to modify the game. Before you get to downloading Hydra, you first need to install BepInEx. There are many variants of BepInEx, we will need the il2cpp version specifically as Among Us is uses the il2cpp compiler for cross-platform compatability. You can download BepInEx from the [Releases](https://github.com/MrDiamond64/Hydra/releases) tab, or you can alternatively get BepInEx binaries from [the BepInEx site](https://builds.bepinex.dev/projects/bepinex_be) if you would like. You may notice that BepInEx Il2Cpp has two architectures: x86 and x64, this is important as Among Us can be x86 or x64 depending on where you downloaded it from. The archeticture of BepInEx must match the architecture of your Among Us installation. As a general rule of thumb, Microsoft Store and Epic Games provide x64 builds of Among Us, and Steam and Itch.io provide x86 builds of the game. If you are still unsure, you can open Task Manager by pressing `ctrl` + `shift` + esc` on your keyboard, finding Among Us in the running processes list, and see if it says "Among Us.exe (32-bit)", if it does not then your build of Among Us us x64.

Once you have downloaded the proper BepInEx for your Among Us build, you want to open your Among Us installation directory (where `Among Us.exe` and `GameAssembly.dll` are located), and extract the contents of BepInEx into the Among Us directory. Alongside the `Among Us.exe` file, you should see new files such as `winhttp.dll`, `BepInEx`, and `dotnet`. If you do not see those files then your archival program may have extracted them into its seperate folder than the current directory, if that is the case then open the newly created folder, and drag and drop all the contents into the same directory that `Among Us.exe` is located at. If everything was done correctly, then you have successfully downloaded BepInEx.
